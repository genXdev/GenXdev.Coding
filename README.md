...
### FEATURES
* ✅ Allows you to define Refactors;
     * ✅ Define a PowerShell script for automatic file selections based on modified date
     * ✅ Allows you to define an LLM Prompt that is then used for selecting files to refactor using a local or cloud based LLM
     * ✅ Allows you just say 'Start-NextRefactor' to continue where you left off
     * ✅ Refactorsets can also have a priority, so you always start refactoring the most important files/refactor paradigm first
* ✅ Allows you to modify any GenXdev cmdlet, simply with 'Assert-GenXdevCmdlet' or it's unit-test with 'Assert-GenXdevCmdletTest'
     * ✅ With 'Assert-NextGenXdevCmdlet' you can easily go through all GenXdev cmdlets in a Module, or even without a module
          selection and just improve them using Visual Studio Code Github CoPilot plugin, one-by-one, using prompt templates
          with auto invocation in vscode.
* ✅ Open a selection of files in Visual Studio Code with 'l *.tests.ps1 | vscode'
      or 'ls *.ps1 -file -rec | vscode'
