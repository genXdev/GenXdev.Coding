...
process {
        ...
        if (-not [string]::IsNullOrWhiteSpace($BaseModuleName)) {
           ...
           if (-not (Microsoft.PowerShell.Core\Get-Module -Name $BaseModuleName -ErrorAction SilentlyContinue)) {
                ...
                $moduleRootDirectory = GenXdev.FileSystem\Expand-Path `
                    -Path "$modulesPath\$BaseModuleName\"
        ...
        if (-not [string]::IsNullOrWhiteSpace($BaseModuleName)) {
            ...
            $filePath = GenXdev.FileSystem\Expand-Path `
                "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\Functions\$ModuleName\$CmdletName.ps1" `
                -DeleteExistingFile -CreateDirectory

            $testFilePath = GenXdev.FileSystem\Expand-Path `
                "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\Tests\$ModuleName\$CmdletName.Tests.ps1" `
                -DeleteExistingFile -CreateDirectory
...
"@ | Microsoft.PowerShell.Utility\Out-File -FilePath $filePath
            ...
            $ModuleManifestPath = GenXdev.FileSystem\Expand-Path "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\$BaseModuleName.psd1"
            ...
            $ModuleManifest.FileList += (GenXdev.FileSystem\Find-Item $filePath -RelativeBasePath (GenXdev.FileSystem\Expand-Path "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\")).Substring(2)
            $ModuleManifest.FileList += (GenXdev.FileSystem\Find-Item $testFilePath -RelativeBasePath (GenXdev.FileSystem\Expand-Path "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\")).Substring(2)
            ...
            $PsmFilePath = GenXdev.FileSystem\Expand-Path "$PSScriptRoot\..\..\..\..\$BaseModuleName\1.156.2025\$ModuleName.psm1"
