    ...
    begin {
        try {
            ...
            if ($Integrate) {
                ...
                $destination = GenXdev.FileSystem\Expand-Path "$PSScriptRoot\..\..\..\..\..\Modules\$baseDestinationModule\1.214.2025\Functions\$ModuleName\$CmdletName.ps1" -CreateDirectory
                ...
                GenXdev.Coding\SplitUpPsm1File -Path "$PSScriptRoot\..\..\..\..\..\Modules\$baseDestinationModule\1.214.2025\$ModuleName.psm1"
    ...
    process {
        try {
            ...
            $invocationParams.KeysToSend = @(
                @("^``", "^``", '^+i', '^l', '^a', '{DELETE}', '^+i', '{ESCAPE}', '^+%{F12}', '{ENTER}', '^v', '{ENTER}', '^{ENTER}',"^``")
            )
