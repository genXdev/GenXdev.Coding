...
function Assert-RefactorFile {
    ...
    begin {
        ...
        if ($Path.ToLowerInvariant().StartsWith($scriptsPath.ToLowerInvariant())) {
            ...
            $functionsPath = GenXdev.FileSystem\Expand-Path "$modulesPath\$baseModuleName\1.190.2025\Functions\" -CreateDirectory
            $testsPath = GenXdev.FileSystem\Expand-Path "$modulesPath\$baseModuleName\1.190.2025\Tests\" -CreateDirectory
