    ...
    process {
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*.ps1`" -PassThru -Exclude `"*\_*`" | Microsoft.PowerShell.Utility\Sort-Object -property LastWriteTime" `
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*.cs`" -PassThru  -Exclude `"*\_*`" | Microsoft.PowerShell.Utility\Sort-Object -property LastWriteTime" `
                -AutoAddModifiedFiles `
                -Priority 0
        }
        ####
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -PromptKey "OnlyDocumentation" `
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*.ps1`" -PassThru -Exclude `"*\_*`" -Quiet -NotMatch -Content `"SYNOPSIS`" | Microsoft.PowerShell.Utility\Sort-Object -property LastWriteTime" `
                -AutoAddModifiedFiles `
                -Priority 2000
        }
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -PromptKey "OnlyCSharpDocumentation" `
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*.cs`" -PassThru -Exclude `"*\_*`" -Quiet -NotMatch -Content `"SYNOPSIS`" | Microsoft.PowerShell.Utility\Sort-Object -property LastWriteTime" `
                -AutoAddModifiedFiles `
                -Priority 2000
        }
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*-*.ps1`" -PassThru -Exclude `"*\_*`", `"*Ensure*`" | ? { `$dir = [io.path]::GetDirectoryName(`$_); `$fn = [io.path]::GetFileNameWithoutExtension(`$_); if (-not (Test-Path `"`$dir\`${fn}.cs`")) { if (-not ([IO.file]::ReadAllText(`$_).Contains('dontrefactor'))) { `$_ } } } | Sort-Object Length" `
        ...
        if (-not $obj) {
            ...
            $null = GenXdev.Coding\New-Refactor `
                ...
                -SelectionScript "GenXdev.FileSystem\Find-Item `"$PSScriptRoot\..\..\..\..\GenXdev.*\3.23.2026\functions\*.cs`" -PassThru -Exclude `"*\_*`" -Quiet -Content 'Confirm-InstallationConsent|Get-Variable|Copy-IdenticalParamValues|Expand-Path|LOCALAPPDATA|\.\.\\\.\.\\|\\Scripts' | Sort-Object Length" `
                -AutoAddModifiedFiles:$false `
                -Priority 1500
        }
